...
# フレームワーク設計案: Generative Ansatz Search (GAS) (改訂版)
...
* **`Runner` (実行エンジン)**
    * **役割**: `Strategy`の探索ループを駆動させることに特化したコンポーネント。
    * **実装**: `Controller`から`Strategy`と`Context`を受け取り、終了条件を満たすまで探索ループを駆動する。ループの各ステップで、`Strategy.step()`を呼び出して**状態の更新内容（updates）を取得し、その結果を`Context`に適用する**。これにより、状態管理の責務が`Runner`に集約され、`Strategy`は計算ロジックに専念できる。

* **`Strategy` (探索戦略)**
    * **役割**: 遺伝的アルゴリズムやモンテカルロ木探索といった、特定の探索アルゴリズムの**核心ロジック**をカプセル化する。
    * **実装**: 探索の「1ステップ」における**状態遷移ロジック**を定義する`step`メソッドを持つ。`step`は現在の`Context`から必要な状態を受け取り、`Generator`や`Evaluator`と協調して**状態の更新内容（updates）を計算し、それを返す**。`Context`の直接的な書き換えは行わず、計算に特化する（`optax`の設計思想に類似）。

* **`Generator` (仮説生成器)**
    * **役割**: 検証可能な仮説（Ansatz）を生成する**純粋な計算コンポーネント**。
    * **実装**: `Strategy`から現在の`Context`を受け取り、LLMを活用して新たなAnsatzを生成し、その結果を`Strategy`に返す。状態の読み書きは直接行わない。

* **`Evaluator` (仮説評価器)**
    * **役割**: 与えられたAnsatzの有効性を評価する**純粋な計算コンポーネント**。
    * **実装**: `Strategy`からAnsatzを受け取り、定量的・定性的評価を実行してスコアを`Strategy`に返す。状態の記録は直接行わない。
...
C4Component
    ...
    ContainerDb(primary_db, "Primary Datastore", "Database", "探索セッションの状態を永続化")
    System_Ext(llm, "LLM (基盤モデル)")
    System_Ext(solvers, "Optimizer/SMT Solver", "計算実行環境")

    Container_Boundary(command_service, "I. Command Service") {
        Component(controller, "Controller", "ライフサイクル管理", "探索プロセスの開始・中断・永続化を管理")
        Component(runner, "Runner", "実行エンジン", "探索ループを駆動し、状態を更新")
        Component(strategy, "Strategy", "探索戦略", "状態更新の内容を計算")
        
        Component(generator, "Generator", "仮説生成器", "LLMを用いて仮説を生成")
        
        Boundary(evaluator_boundary, "Evaluator (仮説評価器)") {
            Component(quantitative_eval, "定量的評価モジュール", "OptimizerやSMT solverで評価")
            Component(qualitative_eval, "定性的評価モジュール", "LLMを用いて簡潔さ等を評価")
        }

        Component(context, "Context", "探索状態管理 (Originator)", "インメモリでの探索状態を一元管理")
        Component(repository, "Repository", "永続化層 (Caretaker)", "Contextの状態をDBに永続化/復元")
    }

    %% --- Top-level Control Flow ---
    Rel(ui_service, controller, "探索を要求", "API Call")
    Rel(controller, runner, "実行を指示")
    Rel(controller, repository, "永続化/復元を指示")

    %% --- Persistence Flow ---
    Rel(repository, context, "状態を永続化/復元")
    Rel(repository, primary_db, "読み書き")

    %% --- Core Execution Loop (Corrected) ---
    Rel(runner, strategy, "ステップ実行を指示し、更新内容を取得")
    Rel(runner, context, "取得した内容で状態を更新")
    Rel(strategy, context, "状態を参照")
    
    %% --- Strategy's Dependencies ---
    Rel(strategy, generator, "利用", "仮説生成")
    Rel(strategy, quantitative_eval, "利用", "定量的評価")
    Rel(strategy, qualitative_eval, "利用", "定性的評価")

    %% --- External Dependencies ---
    Rel(generator, llm, "利用")
    Rel(qualitative_eval, llm, "利用")
    Rel(quantitative_eval, solvers, "利用")
