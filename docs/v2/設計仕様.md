...
仮説生成と評価のサイクルを駆動し、解を探索するメインサービス。asyncioを活用し、LLM APIやGPUなどのI/Oバウンドなタスクを効率的に処理する。ストラテジーパターンにより、多様な探索アルゴリズムに対応可能な設計を持つ。
...
* **`Runner` (実行エンジン)**
    ...
    * **実装**: `Controller`から`Strategy`と`Context`を受け取り、終了条件を満たすまで`Strategy.step()`の呼び出しを繰り返す。探索の状態は`Context`オブジェクトに集約され、ループ完了または中断時に`Controller`へ最終状態を返す。

* **`Strategy` (探索戦略)**
    * **役割**: 遺伝的アルゴリズムやモンテカルロ木探索といった、特定の探索アルゴリズムの**核心ロジック**をカプセル化する。
    * **実装**: 探索の「1ステップ」を定義する`step`メソッドを持つ。`step`は現在の`Context`を受け取り、`Generator`に仮説生成を、`Evaluator`に評価を指示し、アルゴリズムに基づいて更新された`Context`を返す。
...
```mermaid
C4Component
    title Level 3: Component Diagram for Command Service (Refined)
    ...
    Container_Boundary(command_service, "I. Command Service") {
        ...
        Component(strategy, "Strategy", "探索戦略", "GAやMCTS等のアルゴリズムの核心ロジック")
    ...
    %% --- Core Execution Loop ---
    Rel(runner, strategy, "ステップ実行を指示")
    Rel(strategy, context, "状態を参照/更新")
