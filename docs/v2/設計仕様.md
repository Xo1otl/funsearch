...
「世代」や「サイクル」という概念は、物理的な実装から切り離し、Orchestratorが実行する**論理的な「戦略(Strategy)」または「ポリシー(Policy)」**として扱うことが可能かな

# Thinking 2
controlLoopの中で、実はstateを二種類考える必要があることがわかったかもしれない
control stateと、domain stateの二種類である
control stateは、例えば非同期島モデルGAにおけるpending islandなどの情報である

また、それぞれのstateの更新場所は必要十分に定まっている
* control state: task1 reqの直前と、task2 resの直後
* domain state: 

並列処理について、判断はstateに基づくべきだが、polling方式だと、すべての島が進化中という状態を無駄にチェックし続けることになる。そのため効率的で意味のあるタイミングとして、task2完了のイベント発生時にだけ行う。
ただし、task1が終了したらすぐにtask2に

# Your Task
Thinkingを経て、得られた新しい設計モデルをドキュメント化したい。
