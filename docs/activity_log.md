...
jaxを64bitにしたらlbfgsで見つからないバグ治った、やっぱこっちで十分
...
現在、再実装したプログラムを使って、論文で示されている公式の再発見とスコアの検証を完了しました。併せて、並列化対応や様々なオプティマイザへの対応やプロンプトの改善など、基本的な改良も実施済みです。自身の研究テーマと関連して、周期分極反転構造における、非線形結合モード方程式にMgO:SLTの屈折率分散を含めた式を積分することで得られる変換効率の、波長と分極反転ドメイン幅依存性の近似式の同定を行い、LLM-SRでは探索に失敗したものの、自作のライブラリで探索に成功しております。

つきましては、今後の作業方針についてお伺いできればと存じます。現在、以下のタスクを検討しておりますが、改善のための検証を行う上で取り組むべき内容について、ご回答いただける範囲でアドバイスをいただけますと幸いです。

1. 自分の研究テーマに特化した改良・検証
    * 広帯域なSHGデバイス実現に向けた理想的な材料特性の探索
    * データセットを用いず、変換効率のスペクトル分布を計算してフラットさを定量的に測る evaluator の作成
    * 状況に応じた対数誤差の検討など
2. 論文にはない性能比較・分析
    * クラスタリング条件の変更による性能比較 (データセットごとに閾値設定、スコアの範囲指定によるクラスタリング等)
    * 島ごとのスコアの遷移の追跡・分析
3. 発見された関数の定性的な評価
    * 関数の結果を図示することによる可視化・解釈
4. 論文で言及されている各種性能評価を参考にした、改良できそうなハイパーパラメータの調整
    * 島の数、温度、LLMに送信するサンプル数などを変えて発見までの試行回数が減らせるか試す
5. 自身の研究テーマに関連した発見された関数について、本当に事前知識がなかったのか調査
    * 対話的な質問で関連知識も全くないことを確認しているが、モデルのPPLを計算してより厳密に評価
6. 非同期型の島モデルの実装
    * 現在同期型島モデルを行っているが、非同期型島モデルを実装する
...
事前知識もあってバグコードを全く生成しないGemmaよりも、何の知識もなくイデントやカッコやMAX_NPARAMSをたまに無視してくるqwenのほうが探索効率が良さげなのは興味深い、ちょっとやんちゃなほうがいい結果発見できる可能性が高いってコト！？
