...
現在、再実装したプログラムを使って、論文で示されている公式の再発見とスコアの検証を完了しました。併せて、並列化対応や様々なオプティマイザへの対応やプロンプトの改善など、基本的な改良も実施済みです。自身の研究テーマと関連して、非線形結合モード方程式にMgO:SLTの屈折率分散式を含めた方程式に対して、LLM-SRでは探索に失敗したものでも自作のライブラリで探索に成功しております。

つきましては、今後の作業方針についてお伺いできればと存じます。現在、以下のタスクを検討しておりますが、改良のための検証を行う上で取り組むべき内容について、ご回答いただける範囲でアドバイスをいただけますと幸いです。

1. 自分の研究テーマに特化した改良・検証
    * 広帯域なSHGデバイス実現に向けた理想的な材料特性の探索
    * 物理的な特徴量を多く含んだ専用のデータセットの構築、evaluatorにおける対数誤差の検討など
2. 論文にはない性能比較・分析
...
5. 発見された自分の研究テーマの関数の精度がとても良いため、本当に事前知識がなかったのか調べる
    * 自然言語での質問で関連知識も全くないことを確認しましたが、モデルのPPLを計算してより厳密に調べます
