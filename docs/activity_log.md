...
現在、再実装したプログラムを使って、論文で示されている公式の一部の再発見とスコアの検証を完了しました。併せて、並列化対応や多用なオプティマイザへの対応やプロンプトの改善などの基本的な改良を実施しパフォーマンスの向上を確認しました。自身の研究テーマと関連して、周期分極反転構造における非線形結合モード方程式に、MgO:SLTの屈折率分散を含めた式を積分することで得られる変換効率の、波長と分極反転ドメイン幅依存性の近似式の同定を行い、LLM-SRでは失敗しましたが、自作のライブラリを用いることで探索に成功しました。
...
1. 自分の研究テーマに特化した改良・検証
    ...
    * 状況に応じた対数誤差の検討など
2. 発見された関数の定性的な評価
    * 教科書の公式とは異なる計算で高精度に近似できる式が発見されているため、物理的な意味を考える
3. 論文にはない性能比較・分析
...
5. 非同期型の島モデルの実装
    * 現在同期型島モデルを行っているが、非同期型島モデルに改良する
6. 発見された関数について、本当に事前知識がなかったのか調査
    * 進化の過程や事前の対話質問から関連知識を含めて全く事前知識がないことを確認しているが、論文のPPLの式を使ってより厳密に評価
