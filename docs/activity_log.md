...
現在、再実装したプログラムを使って、論文で示されている公式の一部の再発見とスコアの検証を完了しました。併せて、並列化対応や様々なオプティマイザへの対応やプロンプトの改善などの基本的な改良を実施し、パフォーマンスの向上を確認しました。自身の研究テーマと関連して、周期分極反転構造における非線形結合モード方程式に、MgO:SLTの屈折率分散を含めた式を積分することで得られる変換効率の、波長と分極反転ドメイン幅依存性の近似式の同定を行い、LLM-SRでは探索に失敗しましたが、自作のライブラリでは探索に成功しました。
