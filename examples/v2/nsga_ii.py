...
# ==============================================================================
# 1. Core Data Structures & Protocols (GAS Framework Specification)
# ==============================================================================

# --- データ構造 ---
type Individual = np.ndarray
type StrategyState = Any  # 今回は使用しないが、フレームワークの規約として定義
...
type EvaluateFn = Callable[[List[Individual]], Evidence]
...
class Strategy(Protocol):
    ...
    def init(self, strategy_state: StrategyState) -> None:
        """戦略の内部状態を初期化する (今回は未使用)"""
        ...

    def step(self, evidence: Evidence, search_state: SearchState) -> Dict[str, Any]:
        """評価結果と現在の探索状態に基づき、状態の更新内容(updates)を計算する"""
        ...
...
def new_zdt1_evaluate_fn(n_vars: int) -> EvaluateFn:
    """
    GAS: EvaluateFn Factory
    ZDT1問題の評価関数を生成するファクトリー。
    """
    ...
    def evaluate_fn(candidates: List[Individual]) -> Evidence:
...
class NSGAGenerator:
    ...
    def generate_next_population(self, search_state: SearchState) -> List[Individual]:
        """GenerateFnの本体。探索状態に応じて次世代の候補を生成する。"""
...
def new_nsga_generate_fn(population_size: int, n_vars: int, crossover_rate: float, mutation_rate: float, eta_c: float, eta_m: float, bounds: Tuple[float, float]) -> GenerateFn:
    """
    GAS: GenerateFn Factory
    NSGAGeneratorのインスタンスを生成し、GenerateFnとして利用可能な関数を返す。
    """
    gen = NSGAGenerator()
...
class NSGAStrategy:
    ...
    def init(self, strategy_state: StrategyState) -> None:
        pass  # NSGA-IIのStrategyは状態を持たないため、何もしない
    ...
    def _fast_non_dominated_sort(self, population: List[ScoredIndividual]) -> List[List[ScoredIndividual]]:
        ...
        # NOTE: 以下のpop_mapはパフォーマンスとロジックの正確性のために不可欠です。
        # 高速なインデックス検索を実現しています。この行は変更しないでください。
    ...
    def step(self, evidence: Evidence, search_state: SearchState) -> Dict[str, Any]:
        """
        評価結果と現在の探索状態から、次世代の状態への更新内容(updates)を計算する。
        このメソッドはステートレスであり、内部状態を変更しない。
        """
...
def new_nsga_strategy(population_size: int) -> Strategy:
    """
    GAS: Strategy Factory
    NSGAStrategyのインスタンスを生成する。
    """
...
class Orchestrator:
    ...
    def _apply_updates(self, search_state: SearchState, updates: Dict[str, Any]):
        """Strategyが計算した更新内容をSearchStateに適用する"""
    ...
    def run(self, generate_fn: GenerateFn, evaluate_fn: EvaluateFn, strategy: Strategy, search_state: SearchState, max_generations: int):
        ...
        history = []
        strategy.init(None)  # StrategyStateを初期化 (今回は未使用)
        ...
        while search_state.generation < max_generations:
            ...
            updates = strategy.step(evidence, search_state)
